...
export const UsuarioActual = () => {
  ...
  const obtenerPerfiles = async () => {
    const resultado = await PrivateService.peticionGET( ApiBack.PERFILES_OBTENER );
  ...
  const enviarFormulario = async (fh: formaHtml) => {
    ...
    if (formulario.checkValidity() === false) {
      ...
      const resultado = await PrivateService.peticionPUT( urlActualizar, objetoActualizar );

      if (resultado.nuevo) {
        setEnProceso(false);
        MessajeTostify("success", "Usuario actualizado correctamente", 7000);
      } else {
        MessajeTostify( "error", "No se puede actualizar el usuario. Verifique el correo electrónico", 7000 );
      }
    }
  };
  // *******************************************************************
